...
export const DeleteCinemaSchema = z.object({
  id: z.string().min(1, { message: "Cinema ID is required" }),
  name: z.string().optional(),
  userId: z.string(),
});

export type DeleteCinemaType = z.infer<typeof DeleteCinemaSchema>;

export type DeleteCityProps<TData extends Cinema> = {
  cinema: Row<TData>;
  className?: string;
};

export function DeleteCinemaForm<TData extends Cinema>({
  cinema,
  className,
  ...
  const form = useZodForm({
    ...
    defaultValues: {
      id: cinema.original.id,
      name: cinema.original.name,
      userId: session.data?.user?.id ?? "",
    },
  });

  const mutation = useMutation({
    mutationFn: async (values: DeleteCinemaType) => {
      // const result = await deleteCinemaAction(values);
    ...
    onError: (error: Error) => {
      ...
      toast.error(
        error.message || "An error occurred while deleting the cinema",
      );
    },
  });
            ...
            <div className="flex flex-col gap-1 rounded-lg border border-muted p-4">
              <div className="flex items-center space-x-4">
                <span className="text-muted-foreground">
                  {cinema.original.identifier}
                </span>
                <span>{cinema.original.name}</span>
                {/* {status.icon && (
