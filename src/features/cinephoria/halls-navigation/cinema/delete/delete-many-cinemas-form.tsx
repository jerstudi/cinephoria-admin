...
import { deleteManyCinemasAction } from "./delete-cinema.actions";

export const DeleteManyCinemasSchema = z.array(
  z.object({
...
export type DeleteManyCitiesType = z.infer<typeof DeleteManyCinemasSchema>;

type ActionResult =
  | { success: true; serverError?: never }
  | { success?: never; serverError: string };

export type DeleteCinemasProps<TData> = {
  cinemas: Cinema[];
  table?: TableOrigin<TData>;
  className?: string;
};

export function DeleteManyCinemas<TData>({
  cinemas,
  table,
  className,
}: DeleteCinemasProps<TData>) {
  ...
  const mutation = useMutation({
    mutationFn: async (values: DeleteManyCitiesType) => {
      const result = (await deleteManyCinemasAction(values)) as ActionResult;
    ...
    onSuccess: async (deletedMovies) => {
      ...
      // router.refresh();
      toast.success("Cinemas deleted successfully");
    },
    onError: (error: Error) => {
      console.error("Mutation error:", error);
      toast.error(
        error.message || "An error occurred while deleting the cinemas",
      );
      setOpen(false);
      removeRowChecked();
      if (table) {
  ...
  const rowCheckedIds = rowChecked.map((cinema) => cinema.id);
  const selectedCities = cinemas
    .filter((cinema) => rowCheckedIds.includes(cinema.id))
            ...
            <div className="flex w-full items-center justify-between gap-2">
              ...
              <div className="flex items-center">
                <span className="text-xs font-light text-zinc-300">
                  {rowChecked.length}{" "}
                  {rowChecked.length > 1 ? "cinemas" : "cinema"}
                </span>
              </div>
            </div>
            <div>
        ...
        >
          <div className="flex w-full flex-col gap-4">
            <div className="font-semibold">
              Are you sure to delete these cinemas ?
            </div>
            <div className="flex flex-col gap-1 rounded-lg border border-muted p-4">
              <div className="flex">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-start">Cinemas</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    <ScrollArea className="h-1/2 md:h-72">
                      {selectedCities.length > 0 ? (
                        selectedCities.map((cinema) => (
                          <TableRow
                            key={cinema.id}
